...
from optimizer import Krylov_optimizer
...
net = CGNet(model, CrossEntropyLoss(),
            Krylov_optimizer(logger=logger, cfg=config))
...
def train(model, train_loader, epoch, internal=50):
    ...
    for im, label in train_loader:
        ...
        im = np.array(im)
        out_np_s, loss_np_s = model.fit_train(im, label.numpy(), epoch, total_batch_num)
...
def test(model, test_loader, epoch):
    ...
    for im, label in test_loader:
        ...
        im = np.array(im)
        label = label.numpy()
        out_np, loss_np = model.predict(im, label)
        pred_out = np.asnumpy(out_np).argmax(axis=1)
        correct += (pred_out == label).sum()
        test_loss += loss_np.item()
        total += 1

    test_loss /= total
    logger.info("Test loss {}".format(test_loss))
    logger.info("Accuracy  {}".format(100. * correct / len(test_loader.dataset)))
